...
RegisterServerEvent('danglr-wagons:server:buywagon', function(name, price, model, storage, weight)
    local src = source
...
RegisterServerEvent('danglr-wagons:server:ownedwagons', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    ...
    MySQL.query('SELECT `wagonid`, `name`, `storage`, `active` FROM `player_wagons` WHERE `citizenid` = ?', {
        ...
        if response then
            ...
            TriggerClientEvent('danglr-wagons:client:ownedwagons', src, storeWagons)
        end
    end)
end)

RegisterServerEvent('danglr-wagons:server:activatewagon', function(data)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
...
RegisterServerEvent('danglr-wagons:server:spawnwagon')
AddEventHandler('danglr-wagons:server:spawnwagon', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    local isActive = 1
...
RegisterServerEvent('danglr-wagons:server:updatetempwagon')
AddEventHandler('danglr-wagons:server:updatetempwagon', function(tempwagon)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    local isActive = 1
...
RegisterServerEvent('danglr-wagons:server:sellwagon', function(wagon)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local model = nil
    local price = 0
...
RegisterServerEvent('danglr-wagons:server:checkownedwagons', function(wagon)
    local src = source
...
RegisterServerEvent('danglr-wagons:server:tradewagon', function(id, wagon)
    local src = source
    local player = RSGCore.Functions.GetPlayer(src)
